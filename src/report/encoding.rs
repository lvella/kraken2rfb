...
mod tests {
    ...
    fn test_field_formatting() {
        ...
        for (value, precision, expected) in test_cases {
            let decimal_field = Field::DecimalNumber { value: &value, precision };
            let formatted = decimal_field.to_string();
            println!("Decimal {} with precision {} formatted: {}", value, precision, formatted);
    ...
    fn test_write_register_row() {
        ...
        let fields = vec![
            ...
            Field::AlphaNumber { value: "José Silva" },
            Field::AlphaNumber { value: "12345678912" },
            Field::AlphaNumber { value: "01238578455" },
        ];
        write_register_row(&mut writer, &fields).unwrap();
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), "I550|José Silva|12345678912|01238578455\r\n");

        // Test with empty fields using the Empty variant
        let mut writer = Cursor::new(Vec::new());
        let fields = vec![
            Field::AlphaNumber { value: "I550" },
            Field::AlphaNumber { value: "João Silva" },
            Field::Empty,
            Field::AlphaNumber { value: "96325874177" },
        ];
        write_register_row(&mut writer, &fields).unwrap();
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), "I550|João Silva||96325874177\r\n");
        ...
        write_register_row(&mut writer, &fields).unwrap();
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), "I550|||\r\n");
        ...
        let fields = vec![
            Field::AlphaNumber { value: "I550" },
            Field::DecimalNumber { value: &value1, precision: 2 },
            Field::DecimalNumber { value: &value2, precision: 2 },
        ];
        write_register_row(&mut writer, &fields).unwrap();
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), "I550|1234,56|0,01\r\n");
        ...
        write_register_row(&mut writer, &fields).unwrap();
        assert_eq!(String::from_utf8(writer.into_inner()).unwrap(), "I550|31122023\r\n");
    }
}
